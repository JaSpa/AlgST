...
ambiguousUsage loc _k _name amb =
  D.err "ambigous name"
    & D.primary loc "this usage is ambigous"
  ...
  where
    ...
    description = \case
      AmbiguousImport _ (ModuleName m) -> "it may refer to the import from" <+> D.showSyntax m
...
unknownImportItem stmtLoc modName scope (itemLoc :@ _item) =
  D.err "unknown import item"
    & D.primary itemLoc msg
    & D.context stmtLoc "in this import statement"
  where
    msg = "this" <+> scopeMsg <+> "is not exported from" <+> D.showSyntax modName
...
unboundName loc kind v =
  D.err ("unbound " ++ nameKind kind v)
    & D.primary loc "this name is neither defined anywhere in scope nor imported"
