...
color c = c `seq` mempty { ansiForeground = Just (SetColor Vivid c) }

-- | Style the background with a vivid color.
bgColor :: Color -> AnsiStyle
bgColor c = c `seq` mempty { ansiBackground = Just (SetColor Vivid c) }

-- | Style the foreground with a dull color.
colorDull :: Color -> AnsiStyle
colorDull c = c `seq` mempty { ansiForeground = Just (SetColor Dull c) }

-- | Style the background with a dull color.
bgColorDull :: Color -> AnsiStyle
bgColorDull c = c `seq` mempty { ansiBackground = Just (SetColor Dull c) }

palette :: Word8 -> AnsiStyle
palette c = c `seq` mempty { ansiForeground = Just (SetPalette c) }

bgPalette :: Word8 -> AnsiStyle
bgPalette c = c `seq` mempty { ansiBackground = Just (SetPalette c) }
...
renderIO h sdoc = do
    ...
    let go = \sds -> case sds of
            ...
            SAnnPush style rest -> do
                ...
                push newStyle
                ANSI.hSetSGR h (styleToSgrs newStyle)
                go rest
            SAnnPop rest -> do
...
data AnsiStyle = SetAnsiStyle
    { ansiForeground  :: Maybe ColorSpec          -- ^ Set the foreground color, or keep the old one.
    , ansiBackground  :: Maybe ColorSpec          -- ^ Set the background color, or keep the old one.
...
styleToRawText = T.pack . ANSI.setSGRCode . styleToSgrs

styleToSgrs :: AnsiStyle -> [ANSI.SGR]
styleToSgrs (SetAnsiStyle fg bg b i u) = catMaybes
  [ Just ANSI.Reset
  , fmap (\s -> colorSpecToSgr ANSI.Foreground s) fg
  , fmap (\s -> colorSpecToSgr ANSI.Background s) bg
  ...
  where
    colorSpecToSgr :: ANSI.ConsoleLayer -> ColorSpec -> ANSI.SGR
    colorSpecToSgr layer spec = case spec of
      SetColor intensity c -> ANSI.SetColor layer intensity c
      SetPalette         c -> ANSI.SetPaletteColor layer c
